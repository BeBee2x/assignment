#ifndef ASSIGNMENT_BANK_H
#define ASSIGNMENT_BANK_H
...
void phoneFormatValid(char array[50]);
void login();
void password_checking(char array[50]);
...
void login(){
    ...
    while(emailExist==-1){
        ...
        scanf(" %[^\n]",&log_email[0]);
        emailExistChecking(log_email);
        if(emailExist==-1){
            printf("Your email is not registered,Try agian\n");
        }
    }
    char log_pass[50];
    log_password=-1;
    while(log_password==-1){
        printf("Enter your password:");
        scanf(" %[^\n]",&log_pass[0]);
        password_checking(log_pass);
        if(log_password==-1){
...
void password_checking(char array[50]){
    int toCheckCount = charCounting(array);
    int sameCounting=0;

    for(int ncc=0; ncc<users ; ncc++){
        int dbEmailCount = charCounting(db[ncc].password);
        ...
        if( toCheckCount == dbEmailCount){
            ...
            for(int gcc=0; gcc<toCheckCount ; gcc++){
                ...
                if(array[gcc] != db[ncc].password[gcc] ){
                    break;
                } else{
                    sameCounting++;
                }
...
void registration(){
    ...
    format_phone=-1;
    invalid_phone=-1;
    while(format_phone==-1 || invalid_phone==-1){
        printf("Enter your phone number:");
        scanf(" %[^\n]",&rePhone[0]);
        phoneFormatValid(rePhone);
        if(format_phone==-1){
            printf("Phone number must start with country code(+959)\n");
        }else if(invalid_phone==-1){
...
void phoneFormatValid(char array[50]){
    int notValid_phone=-1;
    char format[4]={'+','9','5','9'};
    int phone_counter= charCounting(array);
    for (int i = 0; i < 4; i++) {
        if(format[i]!=array[i]){
            notValid_phone=1;
            break;
...
void myStrongPassword(char pass[50]){
    ...
    if(pass_counter>7){
        while (strongPass == -1){
            ...
            if( pass[i] >= 33 && pass[i] <= 42 || pass[i]==64){
        ...
        printf("Password must be at least 8 digits\n");
    }
